...
// 使用 watchEffect 同步 userDetail 和 open 状态变化
watchEffect(() => {
  if (props.open && props.userDetail) {
    selectedRoleIds.value = props.userDetail.roles.map(role => role.id);
  }
});
                ...
                <div class="flex items-start gap-3 p-3">
                  <div class="pt-0.5">
                    <Checkbox
                      ...
                      @update:model-value="(checked: boolean) => handleRoleToggle(role.id, checked)"
                    />
                  </div>
